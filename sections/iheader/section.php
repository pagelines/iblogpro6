...
/*
	...
	Description: A responsive header for your iBlog.
	Class Name: iHeader
	Workswith: header
	Compatibility: 2.2
	Cloning: false
...
class iHeader extends PageLinesSection {
		...
		$opts = array(
			...
			array(
				...
				'opts'		=> array(
					 ...
					 array(
							...
							'opts'			=> array(
								'iblog'			=> array( 'name'	=> __( 'iBlog (Default)', 'pagelines' ) ),
			...
			array(
				...
				'opts'		=> array(
					array(
						'type' 			=> 'image_upload',
						'default'		=> PL_THEME_URL.'/logo.png',
						'key'			=> 'logo_image',
						'label' 		=> __( 'Logo Image (Optional)', 'pagelines' ),
						'help'			=> __( 'Maximum height: 30px', 'pagelines' ),
					),
					array(
						'key'		=> 'navbar_title',
						'type' 		=> 'text',
   ...
   function section_template( $location = false ) {
	...
	$class[] = ( $theme ) ? sprintf( 'pl-color-%s', $theme ) : 'pl-color-iblog';
	...
	$logo_image = ( $this->opt( 'logo_image' ) || $this->opt( 'logo_image' ) != '' ) 
					? sprintf( '<a href="%s" class="logo_image"><img src="%s" alt="%s" /></a>', get_bloginfo( 'url' ), $this->opt( 'logo_image' ), get_bloginfo( 'name' ) ) 
					: false ;
