...
class SuperGrid extends PageLinesSection {
	...
	function section_opts(){
		...
		$options[] = array(
			'title' 	=> __( 'SuperGrid Styles', 'pagelines' ),
			'key'		=> 'supergrid_styles',
			'type'		=> 'multi',
			'col'		=> 2,
			'opts'		=> array(
				array(
				...
				array(
                    'key' 		=> 'supergrid_text_background_color',
                    'type' 		=> 'color',
                    'label' 	=> __('Text Background Color', 'pagelines'),
                    'default' 	=> 'fff'
                ),
				array(
	...
	function section_head(){
		...
		<style>
			...
			<?php echo $this->prefix(); ?> .supergrid-item .grid-item-title{
				background: <?php echo pl_hashify($this->opt('supergrid_text_background_color')); ?>;
			}
   ...
   function section_template() {
		...
		foreach( $posts as $key => $p ){
			...
			if( $p->post_excerpt ) { 
				...
				$excerpt = sprintf('<p class="excerpt">%s</p>', $get_excerpt);
			} else{
				$excerpt = ''; 
			};
			...
			$title = sprintf(
				'<div class="grid-item-title"><h3><a href="%s">%s</a></h3>%s</div>',
				get_permalink( $p->ID ),
				$p->post_title,
				$excerpt
			);
